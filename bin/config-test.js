...
var ReviewerSelector = class {
   ...
   */
  handleNonGroupMemberRule(rules) {
    const fromClause = rules.non_group_members?.from || rules.default?.from;
    const ruleType = rules.non_group_members?.from ? "non_group_members" : "default";
   ...
   */
  handleMultipleGroupsRule(rules, authorGroups) {
   ...
   */
  handleSingleGroupRule(rules, authorGroup) {
    ...
    const ruleIndex = applicableRule ? rules.by_author_group?.indexOf(applicableRule) : void 0;
    if (!fromClause) return null;
    return {
      type: ruleType,
      index: ruleIndex,
   ...
   */
  handleFirstGroupStrategy(rules, firstGroup) {
    const applicableRule = rules.by_author_group?.find(
   ...
   */
  handleMergeStrategy(_rules, authorGroups) {
   ...
   */
  mergeRulesFromGroups(authorGroups) {
    const rules = this.config.selection_rules;
   ...
   */
  executeSelection(fromClause, author, authorGroup, existingReviewers, process3) {
    let selected = [];
    let stepCounter = 1;
    for (const [groupKey, count] of Object.entries(fromClause)) {
      ...
      const existingFromGroups = existingReviewers.filter(
        (reviewer) => candidates.includes(reviewer)
      );
   ...
   */
  resolveGroupSelection(groupKey, _authorGroup) {
    if (groupKey === "*") {
      return this.config.groups.map((g) => g.name);
    }
    if (groupKey.startsWith("!")) {
      const excludeGroups = groupKey.substring(1).split(",").map((g) => g.trim());
      return this.config.groups.map((g) => g.name).filter((name) => !excludeGroups.includes(name));
    }
    return [groupKey];
  }
  /**
   ...
   */
  getCandidatesFromGroups(groupNames) {
    const candidates = [];
   ...
   */
  pickRandomDeterministic(items, n, ignore, seed = 0) {
    const picks = [];
    const candidates = items.filter((item) => !ignore.includes(item));
   ...
   */
  selectFromMultipleGroups(fromClause, author, authorGroup, existingReviewers) {
    const process3 = [];
...
`);
   ...
   */
  checkRuleForDuplicates(fromClause, groupNames, ruleName) {
    const warnings = [];
    ...
    for (const selector of selectors) {
      if (selector.startsWith("!")) {
        ...
        if (explicitGroups.length > 0) {
          ...
          const overlappingGroups = explicitGroups.filter(
            (g) => includedByExclusion.includes(g)
          );
  ...
  findConfigFile(configPath) {
    ...
    const defaultPath = import_node_path.default.join(gitRoot, ".github", "reviewer-lottery.yml");
    if (import_node_fs.default.existsSync(defaultPath)) {
      return defaultPath;
    }
   ...
   */
  findGitRoot(dir) {
    const gitDir = import_node_path.default.join(dir, ".git");
...
`,
  ...
  colorize(text, type2) {
    ...
    const colorMap = {
      title: source_default.bold.blue,
      header: source_default.bold.cyan,
      group: source_default.bold.green,
      percentage: source_default.yellow,
      good: source_default.green,
      warn: source_default.yellow,
      warning: source_default.bold.yellow,
      error: source_default.red,
      info: source_default.gray
    };
    return colorMap[type2](text);
  }
  /**
  ...
  generateScenarios() {
    const scenarios = [];
    const processedUsers = /* @__PURE__ */ new Set();
   ...
   */
  buildUserGroupMapping() {
    const userGroupCount = /* @__PURE__ */ new Map();
    for (const group of this.config.groups) {
   ...
   */
  addGroupScenarios(scenarios, userGroupCount, processedUsers) {
    for (const group of this.config.groups) {
   ...
   */
  addMultiGroupScenarios(scenarios, userGroupCount) {
   ...
   */
  selectRepresentativeUser(group, userGroupCount, processedUsers) {
    for (const username of group.usernames) {
  ...
  async runSingleScenario(scenario) {
    ...
    const successRate = simulationResults.filter((r) => r.length > 0).length / this.simulationRuns;
  ...
  updateSelectionCounts(selectedReviewers, selectionDetails, selectionCount, groupSelectionCount) {
    for (const reviewer of selectedReviewers) {
      ...
      if (!groupSelectionCount.has(selectedAsGroup)) {
        groupSelectionCount.set(selectedAsGroup, /* @__PURE__ */ new Map());
      }
  ...
  getMostSelectedReviewers(selectionCount) {
    if (selectionCount.size === 0) return [];
    const maxCount = Math.max(...selectionCount.values());
    return Array.from(selectionCount.entries()).filter(([_, count]) => count === maxCount).map(([reviewer, _]) => reviewer);
  }
  ...
  displayGroupResults(groupName, groupResults) {
    console.log(`
${this.colorize(groupName, "group")}:`);
  ...
  processGroupSelectionData(result) {
    const selectionsByGroup = /* @__PURE__ */ new Map();
    const crossGroupMembers = /* @__PURE__ */ new Map();
    for (const [
      groupKey,
      reviewerMap
  ...
  displayGroupSelections(groupKey, selections) {
    ...
    const groupPercentage = (groupTotalSelections / this.simulationRuns * 100).toFixed(1);
    console.log(
    ...
    for (const selection of selections) {
      const percentage = (selection.count / this.simulationRuns * 100).toFixed(1);
      const paddedReviewer = selection.reviewer.length <= maxUsernameLength ? selection.reviewer.padEnd(maxUsernameLength) : selection.reviewer;
      const otherGroups = selection.groups.filter((g) => g !== groupKey);
      const multiGroupIndicator = otherGroups.length > 0 ? ` [also in ${otherGroups.join(", ")}]` : "";
      console.log(
        `        ${paddedReviewer}: ${selection.count}/${this.simulationRuns} (${this.colorize(`${percentage}%`, "percentage")})${multiGroupIndicator}`
      );
    }
  }
  getActualSelectionOutcome(result) {
...
${parts.join("\n")}`;
  ...
  getSelectionRuleForAuthor(author, authorGroup) {
    ...
    if (authorGroups.length > 1) {
      ...
      for (const groupName of authorGroups) {
        ...
        const fromClause = groupRule?.from || this.config.selection_rules.default?.from;
        if (fromClause) {
...
${parts.join("\n")}`;
  ...
  sortGroupsByRuleOrder(groups, author, authorGroup) {
  ...
  getApplicableRule(_author, authorGroup) {
    if (!this.config.selection_rules) {
      return null;
    }
    if (authorGroup && this.config.selection_rules.by_author_group) {
